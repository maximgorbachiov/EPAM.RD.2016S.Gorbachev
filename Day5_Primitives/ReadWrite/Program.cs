    ...
    class Program
    {
        ...
        private static IList<Thread> CreateWorkers(ManualResetEventSlim mres, Action action, int threadsNum, int cycles)
        {
            ...
            for (int i = 0; i < threadsNum; i++)
            {
                ...
                Thread thread = new Thread(()=>d()); // TODO: Create a new thread that will run the delegate above here.
        ...
        static void Main(string[] args)
        {
            ...
            threads.AddRange(CreateWorkers(mres, () => { list.Add(1); }, 2, 3));
            threads.AddRange(CreateWorkers(mres, () => { list.Get(); }, 2, 3));
            threads.AddRange(CreateWorkers(mres, () => { list.Remove(); }, 2, 3));
